...
public class FoodOrderUI extends JFrame {
    ...
    public FoodOrderUI(String machineId) {
    ...
    private void handleSubmit() {
        ...
        try {
            ...
            for (Map.Entry<String, int[]> en : cart.entrySet()) {
                String name = en.getKey();
                int qty = en.getValue()[0];
                int price = en.getValue()[1];
                items.add(Map.of(
                        "name", name,
                        "qty", qty,
                        "price", price
            ...
            var res = ApiClient.submitOrder(machineId, items, txtNote.getText());
            int status = res.getInt("status");
            if (status == 200) {
                JOptionPane.showMessageDialog(this, "Đặt món thành công! Vui lòng chờ nhân viên xác nhận.");
                cart.clear();
                txtNote.setText("");
                updateCartUI();
            } else {
                String msg = res.getJSONObject("body").optJSONObject("error") != null
                        ? res.getJSONObject("body").getJSONObject("error").optString("message", "order_failed")
                        : "order_failed";
                JOptionPane.showMessageDialog(this, "Đặt món thất bại: " + msg);
            }
        } catch (Exception ex) {
    ...
    private void handleSubmit() {
        ...
        try {
            ...
            for (var entry : cart.entrySet()) {
                ...
                items.add(java.util.Map.of("name", name, "qty", qty, "price", price));
            }
            var res = ApiClient.submitOrder(machineId, items, txtNote.getText());
            int status = res.getInt("status");
            if (status == 200) {
                String orderId = res.getJSONObject("body").optString("id", "");
                JOptionPane.showMessageDialog(this, "Đặt món thành công! Mã: " + orderId);
                cart.clear();
                txtNote.setText("");
                updateCartUI();
            } else {
                var err = res.getJSONObject("body").optJSONObject("error");
                JOptionPane.showMessageDialog(this, "Đặt món thất bại: " + (err == null ? "error" : err.optString("message")));
            }
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Lỗi kết nối server: " + ex.getMessage());
        }
    ...
    private void handleSubmit() {
        ...
        try {
            ...
            for (Map.Entry<String, int[]> entry : cart.entrySet()) {
                ...
                int qty = entry.getValue()[0];
                int price = entry.getValue()[1];
                items.add(Map.of("name", name, "qty", qty, "price", price));
            }
            var res = ApiClient.submitOrder(machineId, items, txtNote.getText());
            if (res.getInt("status") == 200) {
                JOptionPane.showMessageDialog(this, "Đặt món thành công!\nMã đơn: " + res.getJSONObject("body").optString("id"));
                cart.clear();
                txtNote.setText("");
                updateCartUI();
                dispose();
                ...
                String msg = res.getJSONObject("body").optJSONObject("error") != null
                        ? res.getJSONObject("body").getJSONObject("error").optString("message", "order_failed")
                        : "order_failed";
                JOptionPane.showMessageDialog(this, "Không thể đặt món: " + msg);
            }
        } catch (Exception e) {
